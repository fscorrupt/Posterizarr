...
function PlexExportHistory() {
  ...
  const fetchStatistics = async (silent = false) => {
    ...
    try {
      const response = await fetch(`${API_URL}/plex-export/statistics`);
      if (!response.ok) {
  ...
  const fetchRuns = async () => {
    try {
      const response = await fetch(`${API_URL}/plex-export/runs`);
      if (!response.ok) {
      ...
      if (data.success) {
        ...
        if (data.runs.length > 0 && !selectedRun) {
          setSelectedRun(data.runs[0]); // Select latest run by default
        }
      }
    } catch (error) {
  ...
  const fetchLibraryData = async (runTimestamp = null) => {
    try {
      const url = runTimestamp
        ? `${API_URL}/plex-export/library?run_timestamp=${encodeURIComponent(
            runTimestamp
          )}`
        : `${API_URL}/plex-export/library`;
  ...
  const fetchEpisodeData = async (runTimestamp = null) => {
    try {
      const url = runTimestamp
        ? `${API_URL}/plex-export/episodes?run_timestamp=${encodeURIComponent(
            runTimestamp
          )}`
        : `${API_URL}/plex-export/episodes`;
  ...
  const importCSVs = async () => {
    ...
    setImporting(true);
    try {
      const response = await fetch(`${API_URL}/plex-export/import`, {
        method: "POST",
      });
      ...
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-bold text-theme-text flex items-center gap-2">
            ...
            {t("plexExport.pageTitle", "Plex Export History")}
          </h1>
          <p className="text-theme-muted mt-1">
            {t(
              "plexExport.description",
              "Track and query your Plex library exports over time"
            )}
          </p>
        </div>
...
export default PlexExportHistory;
