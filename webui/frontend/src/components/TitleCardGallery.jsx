...
function TitleCardGallery() {
  ...
  const fetchFolders = async (showToast = false) => {
    ...
    try {
      ...
      // Only auto-select folders that have title cards
  ...
  const deleteTitleCard = async (imagePath, imageName, event) => {
    ...
    if (
      !window.confirm(
        `Möchtest du die Title Card "${imageName}" wirklich löschen?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        toast.success(`Title Card "${imageName}" erfolgreich gelöscht`, {
          duration: 3000,
          position: "top-right",
        });
        ...
        {activeFolder && images.length > 0 && (
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-theme-card border border-theme-primary rounded-lg focus:outline-none focus:ring-2 focus:ring-theme-primary text-white placeholder-gray-500"
            />
          </div>
        )}
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {displayedImages.map((image, index) => (
              ...
              >
                ...
                <div
                  className="relative cursor-pointer aspect-[16/9]"
          ...
          {hasMore && (
            <div className="flex justify-center mt-6">
              <button
                onClick={loadMore}
                className="flex items-center px-6 py-3 bg-theme-card hover:bg-theme-hover border border-theme-primary rounded-lg font-medium transition-colors"
              >
