...
function BackgroundsGallery() {
  ...
  const deleteBackground = async (imagePath, imageName, event) => {
    ...
    if (
      !window.confirm(
        `Do you really want to delete the background "${imageName}"?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        toast.success(`Background "${imageName}" successfully deleted`, {
          duration: 3000,
          position: "top-right",
        });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-theme-muted mt-2">
            Browse and manage your background collection
          </p>
        </div>
